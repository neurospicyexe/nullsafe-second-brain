...
- Verify CouchDB has the document: `curl -u admin:PASSWORD https://db.example.com/obsidian-vault/_all_docs?limit=5`
...
This lets you verify the server is up by visiting `https://mcp.example.com/health` in a browser.
...
curl -u admin:YOUR_COUCH_PASSWORD https://db.example.com/obsidian-vault/_all_docs?limit=10&descending=true
